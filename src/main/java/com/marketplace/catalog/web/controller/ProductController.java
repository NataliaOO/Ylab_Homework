...
/**
 * REST-контроллер для работы с товарами каталога.
 */
@RestController
@RequestMapping("/api/products")
public class ProductController {
    private static final String ERR_ACCESS_DENIED = "Access denied";
    private static final String ERR_ADMIN_REQUIRED = "Admin role required";
    ...
    public ResponseEntity<?> listOrSearch(
        ...
        if (!hasViewAccess(user)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(new ErrorResponse(ERR_ACCESS_DENIED));
        }
    ...
    public ResponseEntity<?> createProduct(@Valid @RequestBody ProductRequest requestBody,
                                           HttpServletRequest request) {
        User user = getAdminOrNull(request);
        if (user == null) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(new ErrorResponse(ERR_ADMIN_REQUIRED));
        }
    ...
    private User getAdminOrNull(HttpServletRequest request) {
        User user = getCurrentUser(request);
        if (!isAdmin(user)) {
            return null;
        }
